	...
	class penci_popular_news_widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if ($loop->have_posts()) :
				...
				$rand = rand(100,10000);
				?>
				<ul id="penci-popularwg-<?php echo sanitize_text_field( $rand ); ?>" class="side-newsfeed<?php if( $twocolumn && ! $allfeatured ): echo ' penci-feed-2columns'; if( $featured ){ echo ' penci-2columns-featured'; } else { echo ' penci-2columns-feed'; } endif;?><?php if ( ! $ordernum ): echo ' display-order-numbers'; endif; ?>">
			...
			if( $image_type == 'horizontal' ){
				echo '<style>#penci-popularwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 66.6667%; }</style>';
			} elseif( $image_type == 'square' ) {
				echo '<style>#penci-popularwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 100%; }</style>';
			} elseif( $image_type == 'vertical' ) {
				echo '<style>#penci-popularwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 135.4%; }</style>';
			}
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => esc_html__('Popular Posts', 'soledad'), 'image_type' => 'default', 'type' => '', 'number' => 5, 'categories' => '', 'featured' => false, 'allfeatured' => false, 'thumbright' => false, 'twocolumn' => false, 'featured2' => false, 'ordernum' => false, 'postdate' => false, 'icon' => false );
