	...
	class penci_latest_news_widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			if( $image_type == 'horizontal' ){
				echo '<style>#penci-latestwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 66.6667%; }</style>';
			} elseif( $image_type == 'square' ) {
				echo '<style>#penci-latestwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 100%; }</style>';
			} elseif( $image_type == 'vertical' ) {
				echo '<style>#penci-latestwg-' . sanitize_text_field( $rand ) . ' .penci-image-holder:before{ padding-top: 135.4%; }</style>';
			}
		...
		function form( $instance ) {
			...
			$defaults = array( 'title' => esc_html__('Recent Posts', 'soledad'), 'image_type' => 'default', 'number' => 5, 'categories' => '', 'featured' => false, 'allfeatured' => false, 'thumbright' => false, 'twocolumn' => false, 'featured2' => false, 'postdate' => false, 'icon' => false );
